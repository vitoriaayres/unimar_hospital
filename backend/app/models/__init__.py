...
class Product(Base):
    ...
    category: Mapped[ProductCategory] = enum_column(
        ProductCategory, nullable=False, default=ProductCategory.OTHER
    )
...
class InventoryBatch(Base):
    ...
    unit_cost: Mapped[float] = mapped_column(nullable=False, default=0.0)
    status: Mapped[BatchStatus] = enum_column(
        BatchStatus, nullable=False, default=BatchStatus.AVAILABLE
    )
...
class StockMovement(Base):
    ...
    user: Mapped[User | None] = relationship(back_populates='stock_movements')
...
class Consumption(Base):
    ...
    prescription_type: Mapped[PrescriptionType] = enum_column(
        PrescriptionType, nullable=False, default=PrescriptionType.ROUTINE
    )
...
class Prediction(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            'product_id', 'forecast_date', 'model_version', name='uq_product_forecast_model'
        ),
    )
    ...
    wape_score: Mapped[float | None] = mapped_column(nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
