        ...
        try:
            ...
            if d:
                rows = [
                    ...
                    (
                        'Estoque Baixo',
                        str(d['low_stock_count']),
                        _mini_bar(d['low_stock_count'], 100, 15),
                    ),
                    (
                        'Risco de Falta',
                        str(d['stockout_risk_count']),
                        f'[red]{_mini_bar(d["stockout_risk_count"], 500, 15)}[/]',
                    ),
                    (
                        'Vencendo (30d)',
                        str(d['expiring_soon_count']),
                        f'[yellow]{_mini_bar(d["expiring_soon_count"], 100, 15)}[/]',
                    ),
                    ('Valor Estoque', f'R$ {d["total_inventory_value"]:,.2f}', ''),
                    ('WAPE Medio', f'{d["average_wape"]:.2f}%', ''),
                    ('Previsoes Hoje', str(d['predictions_generated_today']), ''),
                    (
                        'Alertas Nao Lidos',
                        str(d['alerts_unacknowledged']),
                        f'[red]{_mini_bar(d["alerts_unacknowledged"], 1000, 15)}[/]',
                    ),
                ]
                for k, v, bar in rows:
                    t.add_row(k, v, bar)
            ...
            trend = api_get('/dashboard/consumption-trends', params={'days_back': 30})
            if trend and 'items' in trend:
                ...
                if vals:
                    ...
                    self.query_one('#sparkline-label', Label).update(
                        '  [dim]Consumo diario (ultimos 30 dias)[/]'
                    )
        except Exception as e:
    ...
    def _load_data(self) -> None:
        ...
        try:
            self.__class__._all_data = (
                api_get('/dashboard/stockout-risk', params={'limit': 100}) or []
            )
    ...
    def _render_table(self, data):
        try:
            ...
            for i, item in enumerate(data, 1):
                ...
                c = {'critical': 'red', 'high': 'red', 'medium': 'yellow', 'low': 'green'}.get(
                    risk, 'white'
                )
                t.add_row(
                    str(i),
    ...
    def _apply_filters(self):
        ...
        if search:
            filtered = [
                x
                for x in filtered
                if search in (x.get('product_name') or '').lower()
                or search in (x.get('product_sku') or '').lower()
            ]
...
class AlertasView(Static):
    ...
    def _apply_filters(self):
        ...
        for item in filtered:
            sev_count[item.get('severity', 'info')] = (
                sev_count.get(item.get('severity', 'info'), 0) + 1
            )
        try:
...
class VencimentoView(Static):
    ...
    def _load_data(self) -> None:
        ...
        try:
            self.__class__._all_data = (
                api_get('/dashboard/expiry-timeline', params={'days_ahead': 90, 'limit': 100}) or []
            )
            self._apply_filters()
        except Exception as e:
...
class ConsumoView(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id='filters'):
            yield Input(
                placeholder='Buscar data (ex: 2024-03)...', id='search', classes='filter-input'
            )
        yield DataTable(id='tbl')
        yield Sparkline([], id='spark')
    ...
    def _load_data(self) -> None:
        ...
        self._set_error(None)
        try:
            trend = api_get('/dashboard/consumption-trends', params={'days_back': 60})
            if trend and 'items' in trend:
    ...
    def _apply_filters(self):
        ...
        if search:
            filtered = [
                x
                for x in filtered
                if search in (x.get('movement_type') or '').lower()
                or search in (x.get('batch_id') or '').lower()
            ]
...
CATEGORY_LABELS = {
    ...
    'antithrombotic': 'Antitrombotico',
    'beta_blocker': 'Beta-bloqueador',
    'ppi': 'IBP',
    'bronchodilator': 'Broncodilatador',
    'psycholeptic': 'Psicolptico',
    'ace_inhibitor': 'IECA',
...
class CadastrarProdutoView(Static):
                ...
                with HorizontalScroll(id='cat-buttons'):
                    for cat in CATEGORIES:
                        ...
                        yield Button(
                            CATEGORY_LABELS[cat],
                            id=f'cat-{cat}',
                            classes=f'filter-btn form-btn{active}',
                        )
            with Horizontal(classes='form-row'):
    ...
    def _clear_form(self):
        ...
        for btn in self.query('#cat-buttons .filter-btn'):
            btn.classes = (
                'filter-btn form-btn -active' if btn.id == 'cat-other' else 'filter-btn form-btn'
            )
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if btn_id.startswith('cat-'):
            ...
            for btn in self.query('#cat-buttons .filter-btn'):
                btn.classes = (
                    'filter-btn form-btn -active' if btn.id == btn_id else 'filter-btn form-btn'
                )
        elif btn_id == 'ctrl-no':
            self._selected_controlled = False
            ...
            if api_err:
                ...
                self.query_one('#form-status', Label).update(
                    f'  [green]Produto criado: {result.get("name", "")} (SKU: {result.get("sku", "")})[/]'
                )
...
class RecebimentoView(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        with ScrollableContainer(classes='form-scroll'):
            yield Label(
                '[dim]  Produtos cadastrados (use SKU ou nome para buscar):[/]',
                classes='form-ref-title',
            )
            ...
            with Horizontal(classes='form-row'):
                ...
                yield Input(
                    placeholder='SKU ou nome do produto', id='f-product', classes='form-input'
                )
            with Horizontal(classes='form-row'):
...
class DispensacaoView(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        with ScrollableContainer(classes='form-scroll'):
            yield Label(
                '[dim]  Lotes disponiveis (copie o ID do lote):[/]', classes='form-ref-title'
            )
            ...
            with Horizontal(classes='form-row'):
                ...
                yield Input(
                    placeholder='Observacoes (opcional)', id='f-notes', classes='form-input'
                )
            yield Label('')
            with Horizontal(classes='form-actions'):
...
class RegistroConsumoView(Static):
            ...
            with Horizontal(classes='form-row'):
                ...
                with Horizontal(id='presc-buttons'):
                    for ptype, plabel in PRESCRIPTION_TYPES:
                        ...
                        yield Button(
                            plabel, id=f'presc-{ptype}', classes=f'filter-btn form-btn{active}'
                        )
            yield Label('')
            with Horizontal(classes='form-actions'):
                yield Button('Confirmar', id='btn-confirm', classes='form-btn-confirm')
    ...
    def _clear_form(self):
        ...
        for btn in self.query('#dept-buttons .filter-btn'):
            btn.classes = (
                'filter-btn form-btn -active'
                if btn.id == 'dept-outpatient'
                else 'filter-btn form-btn'
            )
        for btn in self.query('#presc-buttons .filter-btn'):
            btn.classes = (
                'filter-btn form-btn -active'
                if btn.id == 'presc-routine'
                else 'filter-btn form-btn'
            )
...
class PrevisoesView(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id='pred-filters'):
            yield Input(
                placeholder='Buscar produto (nome/SKU)...', id='pred-search', classes='filter-input'
            )
            yield Button(
                'Gerar Previsao', id='pred-generate', classes='filter-btn', variant='primary'
            )
    ...
    def _generate_forecast(self) -> None:
        ...
        try:
            ...
            data = {
                ...
                'horizon_days': 30,
                'model_version': self._selected_model,
            }
            result, err = api_post('/predictions/forecast', data)
            if err:
    ...
    def _render_forecast_table(self, data):
        try:
            ...
            self.query_one('#detail', Label).update(
                ...
                f'  WAPE do modelo: {(summary.get("wape") or 0) * 100:.1f}%'
            )
        except Exception:
            pass
