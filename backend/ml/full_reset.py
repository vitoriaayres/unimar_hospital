...
ENUM_MAP = {
    "UserRole.PHARMACIST": "pharmacist", "UserRole.P": "pharmacist", "pharmacist": "pharmacist",
    "UserRole.MANAGER": "manager", "UserRole.M": "manager", "manager": "manager",
    "UserRole.ADMIN": "admin", "UserRole.A": "admin", "admin": "admin",
    "BatchStatus.AVAILABLE": "available", "BatchStatus.A": "available", "available": "available",
    "BatchStatus.RESERVED": "reserved", "BatchStatus.R": "reserved", "reserved": "reserved",
    "BatchStatus.EXPIRED": "expired", "BatchStatus.E": "expired", "expired": "expired",
    "BatchStatus.RECALLED": "recalled", "BatchStatus.Q": "quarantine", "recalled": "recalled", "quarantine": "quarantine",
    "MovementType.IN": "in", "MovementType.I": "in", "in": "in",
    "MovementType.OUT": "out", "MovementType.O": "out", "out": "out",
    "MovementType.ADJUSTMENT": "adjustment", "adjustment": "adjustment",
    "MovementType.TRANSFER": "transfer", "transfer": "transfer",
    "MovementType.LOSS": "loss", "loss": "loss",
    "MovementType.EXPIRED": "expired",
    "MovementType.RECALLED": "recalled",
    "AlertType.SHORTAGE_RISK": "shortage_risk", "AlertType.S": "shortage_risk", "shortage_risk": "shortage_risk",
    "AlertType.EXPIRY_RISK": "expiry_risk", "AlertType.E": "expiry_risk", "expiry_risk": "expiry_risk",
    "AlertType.OVERSTOCK": "overstock", "AlertType.O": "overstock", "overstock": "overstock",
    "AlertType.REORDER_POINT": "reorder_point", "reorder_point": "reorder_point",
    "AlertSeverity.INFO": "info", "AlertSeverity.I": "info", "info": "info",
    "AlertSeverity.WARNING": "warning", "AlertSeverity.W": "warning", "warning": "warning",
    "AlertSeverity.CRITICAL": "critical", "AlertSeverity.C": "critical", "critical": "critical",
    "PrescriptionType.ROUTINE": "routine", "PrescriptionType.R": "routine", "routine": "routine",
    "PrescriptionType.EMERGENCY": "emergency", "PrescriptionType.E": "emergency", "emergency": "emergency",
    "PrescriptionType.PROPHYLACTIC": "prophylactic", "PrescriptionType.P": "prophylactic", "prophylactic": "prophylactic",
    "Department.ICU": "icu", "Department.I": "icu", "icu": "icu",
    "Department.ER": "er", "Department.E": "er", "er": "er",
    "Department.WARD": "ward", "Department.W": "ward", "ward": "ward",
    "Department.OUTPATIENT": "outpatient", "Department.O": "outpatient", "outpatient": "outpatient",
    "ProductCategory.ANTIBIOTIC": "antibiotic", "ProductCategory.A": "antibiotic", "antibiotic": "antibiotic",
    "ProductCategory.ANALGESIC": "analgesic", "analgesic": "analgesic",
    "ProductCategory.ANTITHROMBOTIC": "antithrombotic", "antithrombotic": "antithrombotic",
    "ProductCategory.BETA_BLOCKER": "beta_blocker", "beta_blocker": "beta_blocker",
    "ProductCategory.PPI": "ppi", "ppi": "ppi",
    "ProductCategory.BRONCHODILATOR": "bronchodilator", "bronchodilator": "bronchodilator",
    "ProductCategory.PSYCHOLEPTIC": "psycholeptic", "psycholeptic": "psycholeptic",
    "ProductCategory.ACE_INHIBITOR": "ace_inhibitor", "ace_inhibitor": "ace_inhibitor",
    "ProductCategory.CORTICOSTEROID": "corticosteroid", "corticosteroid": "corticosteroid",
    "ProductCategory.OTHER": "other", "other": "other",
}
...
def main():
    ...
    for t in ["alerts", "consumption", "stock_movements", "inventory_batches",
              "products", "warehouses", "users", "audit_logs", "predictions",
              "alembic_version"]:
        cur.execute(f"DROP TABLE IF EXISTS {t} CASCADE")
    # Now drop all enum types
    for e in ["userrole", "productcategory", "batchstatus", "movementtype",
              "alerttype", "alertseverity", "prescriptiontype", "department"]:
        cur.execute(f"DROP TYPE IF EXISTS {e} CASCADE")
    # Verify types are gone
    cur.execute("SELECT typname FROM pg_type WHERE typname IN ('userrole','productcategory','batchstatus','movementtype','alerttype','alertseverity','prescriptiontype','department')")
    ...
    cur.execute("UPDATE users SET hashed_password = %s WHERE email = %s",
                (pwd, "admin@hospital.gov.br"))
    print(f"  admin@hospital.gov.br / admin123")

    conn.commit()
    print("\nStep 4: Verifying...")
    for t in ["users", "warehouses", "products", "inventory_batches",
