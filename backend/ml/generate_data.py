...
class SyntheticDataGenerator:
    ...
    def _generate_warehouses(self) -> None:
        ...
        warehouse_data = [
            ...
            {
                'name': 'Farmácia Satélite PS',
                'location': 'Térreo - Pronto Socorro',
                'is_primary': False,
            },
            {
                'name': 'Almoxarifado de Reserva',
                'location': 'Subsolo - Bloco B',
                'is_primary': False,
            },
        ]
    ...
    def _generate_products(self) -> None:
        ...
        for i in range(product_count):
            ...
            max_stock = max(
                min_stock * 2, int(base_demand * self.config['products']['max_stock_multiplier'])
            )
            ...
            lead_time = max(
                self.config['products']['lead_time_days']['min'],
                min(self.config['products']['lead_time_days']['max'], lead_time),
            )

            controlled = (
                self.rng.random() < self.config['products']['controlled_substance_probability']
            )
    ...
    def _generate_pareto_demand_weights(self) -> None:
        ...
        print(
            f'      Pareto: top 20% products = {top_20_pct}/{n_products} = {top_20_sum:.1f}% of demand'
        )
            ...
            for b in range(num_batches):
                ...
                batch = InventoryBatch(
                    ...
                    received_at=datetime.combine(
                        self.start_date - timedelta(days=int(self.rng.integers(1, 30))),
                        datetime.min.time(),
                    ),
                )
            ...
            if self.rng.random() < 0.7:  # 70% chance of secondary outbreak
                ...
                for day_offset in range(resp_duration_days):
                    ...
                    if self.start_date <= current_date <= self.end_date:
                        ...
                        outbreak_mults[current_date] = existing * (
                            1.0 + (resp_peak_multiplier - 1.0) * intensity
                        )
            ...
            if self.rng.random() < 0.5:  # 50% chance of holiday spike
                ...
                for day_offset in range(holiday_duration):
                    ...
                    if self.start_date <= current_date <= self.end_date:
                        ...
                        existing = outbreak_mults.get(current_date, 1.0)
                        outbreak_mults[current_date] = existing * (
                            1.0 + (holiday_multiplier - 1.0) * intensity
                        )
    ...
    def _simulate_daily_operations(self) -> None:
        ...
        pareto_weights = np.array(
            [self._product_demand_weights.get(p.id, 1.0) for p in self.products]
        )
        ...
        atc_keys = list(self.config['products']['class_base_demand_per_100_beds'].keys()) + [
            'OTHER'
        ]
        ...
        print(
            f'      Gerando consumo vetorizado ({n_days} dias x {n_products} produtos x {n_depts} deptos)...'
        )
        ...
        for batch_start in range(0, n_days, batch_size):
            ...
            for d_idx in range(n_depts):
                ...
                seasonal = seasonal_mults[batch_start:batch_end][
                    :, product_atc_idx
                ]  # (batch_days, n_products)
                # Apply Pareto weights for realistic demand heterogeneity
                expected = (
                    base_demands[np.newaxis, :]
                    * dept_weights[d_idx]
                    * dept_mults[d_idx]
                    * seasonal
                    * pareto_scaled[np.newaxis, :]
                )
                ...
                for day_offset in range(batch_days):
                    ...
                    current_date = (
                        sim_date_val.date() if hasattr(sim_date_val, 'date') else sim_date_val
                    )
                ...
                presc_samples = self.rng.choice(
                    presc_indices, size=(batch_days, n_products), p=presc_weights
                )
                ...
                for i in range(len(day_offsets)):
                    ...
                    consumption = Consumption(
                        ...
                        prescription_type=PrescriptionType(
                            presc_types[presc_samples[day_offsets[i], prod_indices[i]]]
                        ),
                        context={
                            'seasonal_multiplier': round(
                                float(seasonal[day_offsets[i], prod_indices[i]]), 3
                            ),
            ...
            if (batch_start // batch_size) % 10 == 0:
                print(
                    f'      ... {batch_end}/{n_days} dias processados ({total_consumptions} consumos)'
                )
    ...
    def _get_seasonal_multiplier(self, current_date: date) -> dict[str, float]:
        ...
        multipliers = dict.fromkeys(
            self.config['products']['class_base_demand_per_100_beds'].keys(), 1.0
        )
        ...
        for atc, patterns in class_seasonality.items():
            if atc in multipliers:
                ...
                for key, value in patterns.items():
                    if key.endswith('_multiplier') and key not in [
                        'winter_multiplier',
                        'summer_multiplier',
                    ]:
    ...
    def _simulate_product_daily_consumption(
        ...
        dept_multipliers = [
            d['consumption_multiplier'] for d in self.config['hospital']['departments']
        ]
        ...
        for dept_id, dept_weight, dept_mult in zip(departments, dept_weights, dept_multipliers):
            ...
            if qty > 0:
                ...
                consumption = Consumption(
                    ...
                    prescription_type=PrescriptionType(
                        self.rng.choice(presc_types, p=presc_weights)
                    ),
                    context={
        ...
        for product in self.products:
            ...
            if available <= product.min_stock_level:
                ...
                if not has_pending:
                    ...
                    lead_time = int(
                        self.rng.normal(supplier['lead_time_mean'], supplier['lead_time_std'])
                    )
                    lead_time = max(
                        supplier['lead_time_min'], min(supplier['lead_time_max'], lead_time)
                    )
                    ...
                    alert = Alert(
                        ...
                        metadata={
                            ...
                            'excess_percentage': round(
                                (current_qty / product.max_stock_level - 1) * 100, 1
                            ),
                        },
                    )
                    self.alerts.append(alert)
    ...
    def _to_dataframes(self) -> dict[str, pl.DataFrame]:
        ...
        schemas = {
            ...
            'products': {
                ...
                'min_stock_level': pl.Int64,
                'max_stock_level': pl.Int64,
                'lead_time_days': pl.Int64,
                'controlled_substance': pl.Boolean,
                'is_active': pl.Boolean,
                'metadata': pl.Utf8,
                'created_at': pl.Utf8,
                'updated_at': pl.Utf8,
            ...
            'inventory_batches': {
                ...
                'batch_number': pl.Utf8,
                'quantity': pl.Int64,
                'expiry_date': pl.Date,
                'manufacture_date': pl.Date,
                'unit_cost': pl.Float64,
                'status': pl.Utf8,
                'received_at': pl.Utf8,
                'created_at': pl.Utf8,
            ...
            'consumption': {
                'id': pl.Utf8,
                'product_id': pl.Utf8,
                'consumption_date': pl.Date,
                'quantity': pl.Int64,
                'department': pl.Utf8,
                'prescription_type': pl.Utf8,
                'context': pl.Utf8,
                'created_at': pl.Utf8,
                'updated_at': pl.Utf8,
            },
            'alerts': {
                ...
                'message': pl.Utf8,
                'metadata': pl.Utf8,
                'acknowledged': pl.Boolean,
                'acknowledged_by': pl.Utf8,
                'acknowledged_at': pl.Utf8,
                'created_at': pl.Utf8,
            },
        }
...
def main():
    ...
    parser.add_argument(
        '--config', default='ml/config/data_generation.yaml', help='Path to config YAML'
    )
