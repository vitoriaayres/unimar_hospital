...
def predict_all(
    model, feature_cols: list[str], features: pl.DataFrame
) -> pl.DataFrame:
    """Gera previsão para todos os produtos (último dia disponível)."""
    latest = (
        features
        .sort("consumption_date", descending=True)
        .group_by("product_id")
        .first()
    )
    ...
    result = latest.select([
        ...
        pl.Series("predicted_consumption_7d", preds)
    )
...
def save_predictions_to_db(
    predictions: pl.DataFrame, model_version: str, mape_score: float
) -> int:
    ...
    # Limpar predicoes anteriores para o mesmo modelo
    cur.execute(
        "DELETE FROM predictions WHERE model_version = %s",
        (model_version,),
    )
        ...
        cur.execute(
            ...
            INSERT INTO predictions
                (id, product_id, forecast_date, predicted_quantity,
                 confidence_lower, confidence_upper, model_version, mape_score, created_at)
...
def main() -> None:
    ...
    print("\n4. Gerando previsoes...")
    predictions = predict_all(model, feature_cols, features)
    ...
    try:
        inserted = save_predictions_to_db(predictions, model_version, mape_score)
    ...
    for _, row in top10.iterrows():
        print(f"   {row['product_id'][:8]}...  consumoAtual={row['daily_consumption']:>6.0f}  prev7d={row['predicted_consumption_7d']:>8.1f}")
