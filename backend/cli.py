...
def d():
    ...
    kpi_table.add_row('WAPE Medio', f'{data["average_wape"]:.2f}%')
...
@click.option(
    '--ordenar-por', '-o', type=click.Choice(['nome', 'sku', 'estoque', 'risco']), default='risco'
)
...
@click.option(
    '--tipo',
    '-t',
    type=click.Choice(['shortage_risk', 'expiry_risk', 'overstock', 'todos']),
    default='todos',
)
def a(limite, tipo):
...
def status():
    ...
    try:
        ...
        data = r.json()
        console.print(
            f'[green]Backend: {data["status"]}[/] (v{data["version"]}, {data["environment"]})'
        )
    except requests.exceptions.ConnectionError:
        console.print('[red]Backend offline.[/]')
