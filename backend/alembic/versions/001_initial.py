...
def upgrade() -> None:
    ...
    enum_definitions = [
        ...
        ("productcategory", ["antibiotic", "analgesic", "antithrombotic", "beta_blocker", "ppi", "bronchodilator", "psycholeptic", "ace_inhibitor", "corticosteroid", "other"]),
    ...
    op.create_table(
        ...
        sa.Column('role', postgresql.ENUM(name='userrole', create_type=False), nullable=False, server_default='pharmacist'),
    ...
    op.create_table(
        ...
        sa.Column('category', postgresql.ENUM(name='productcategory', create_type=False), nullable=False, server_default='other'),
    ...
    op.create_table(
        ...
        sa.Column('unit_cost', sa.Numeric(10, 2), nullable=False, server_default='0.00'),
        sa.Column('status', postgresql.ENUM(name='batchstatus', create_type=False), nullable=False, server_default='available'),
    ...
    op.create_table(
        'stock_movements',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('batch_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('inventory_batches.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='SET NULL'), nullable=True),
        sa.Column('quantity_change', sa.Integer, nullable=False),
        sa.Column('movement_type', postgresql.ENUM(name='movementtype', create_type=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('department', postgresql.ENUM(name='department', create_type=False), nullable=False),
        sa.Column('prescription_type', postgresql.ENUM(name='prescriptiontype', create_type=False), nullable=False, server_default='routine'),
        sa.Column('context', postgresql.JSONB, nullable=False, server_default='{}'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), onupdate=sa.func.now(), nullable=False),
    )
    op.create_index('ix_consumption_product_date', 'consumption', ['product_id', 'consumption_date'])
    ...
    op.create_unique_constraint('uq_product_forecast_model', 'predictions', ['product_id', 'forecast_date', 'model_version'])
...
def downgrade() -> None:
    ...
    # Drop enum types
    for enum_name in ['department', 'prescriptiontype', 'alertseverity', 'alerttype', 'batchstatus', 'movementtype', 'productcategory', 'userrole']:
